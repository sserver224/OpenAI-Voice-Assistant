...
CreateKeyEx(OpenKey(HKEY_CURRENT_USER, 'Software', reserved=0, access=KEY_ALL_ACCESS), 'sserver\OpenAI Virtual Assistant', reserved=0)
...
try:
    ...
    key=simpledialog.askstring('API Key', 'No API key detected. Enter your OpenAI API key, or press Cancel to quit.')
    if key is None:
        sys.exit()
    if key=='':
        sys.exit()
    OPENAI_API_KEY=key
    SetValueEx(OpenKey(OpenKey(OpenKey(HKEY_CURRENT_USER, 'Software', reserved=0, access=KEY_ALL_ACCESS), 'sserver', reserved=0, access=KEY_ALL_ACCESS), 'OpenAI Virtual Assistant', reserved=0, access=KEY_ALL_ACCESS), 'Key', 0, REG_SZ, key)
try:
    key=QueryValueEx(OpenKey(OpenKey(OpenKey(HKEY_CURRENT_USER, 'Software', reserved=0, access=KEY_ALL_ACCESS), 'sserver', reserved=0, access=KEY_ALL_ACCESS), 'OpenAI Virtual Assistant', reserved=0, access=KEY_ALL_ACCESS), 'PVKey')[0]
    if key!='':
        PICOVOICE_API_KEY=key
    else:
        raise OSError
except OSError:
    key=simpledialog.askstring('API Key', 'No API key detected. Enter your PicoVoice API key, or press Cancel to quit.')
    if key is None:
        sys.exit()
    if key=='':
        sys.exit()
    PICOVOICE_API_KEY=key
    SetValueEx(OpenKey(OpenKey(OpenKey(HKEY_CURRENT_USER, 'Software', reserved=0, access=KEY_ALL_ACCESS), 'sserver', reserved=0, access=KEY_ALL_ACCESS), 'OpenAI Virtual Assistant', reserved=0, access=KEY_ALL_ACCESS), 'PVKey', 0, REG_SZ, key)
        ...
        if result == 0:
            ...
            try:
                ...
                try:
                    ...
                    synthesize_and_play_audio("Something doesn't seem to work right now. Let's try again later.")
                else:
                    ...
                    elif text=='volume down' or text=='lower the volume' or text=='make it quieter' or text=='make it softer' or text=='dial it down' or text=='reduce the volume' or text=='decrease the volume':
                        ...
                        try:
                            ...
                            try:
                                ...
                                if text=='yes':
                                    ...
                                    shutdown('s -f')
                    elif text=='reboot' or text=='reboot the device' or text=='reboot the computer' or text=='restart' or text=='restart the device'  or text=='restart the computer':
                        synthesize_and_play_audio("Are you sure you want to restart the computer?")
                        record_audio()
                        try:
                            text = transcribe_audio()
                        except openai.error.APIConnectionError:
                            ...
                            try:
                                ...
                                if text=='yes':
                                    ...
                                    shutdown('r -f')
                    elif text=='Lock' or text=='lock the computer' or text=='lock the device':
